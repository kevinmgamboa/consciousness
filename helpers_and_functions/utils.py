...
def cm_fold_to_df(cm_per_fold, losses):
    """
    ...
    data_f = pd.DataFrame(
        columns=['true_p', 'false_n', 'false_p', 'true_n', 'acc', 'precision', 'recall', 'f1', 'fold', 'loss'])
    for num, cmpp in enumerate(cm_per_fold):
        ...
        data_f = data_f.append(pd.DataFrame(cmpp, columns=['true_p', 'false_n', 'false_p', 'true_n',
                                                           'acc', 'precision', 'recall', 'f1', 'fold', 'loss']),
                               ignore_index=True)
