...
def sleep_montage(raw):
    ...
    new_raw = mne.set_bipolar_reference(raw.load_data(), anode=['Fpz'], cathode=['Cz'])
...
def overlapping(signal, stepsize=1, fs=1000, time_window=0.2, number_time_samples=1):
    ...
    append = np.zeros((number_time_samples - len(
        signal) % number_time_samples))  # this calculates how many samples to add to the input vector for the windows to pdf along it, and creates a zeros vector of that size.
    signal = np.hstack((signal,
                        append))  # completes the input vector with the zeros vector created in order to have an even number of windows pdf in the data
...
#
# -----------------------------------------------------------------------------
def eeg_total_time(dataset, which='anaesthesia'):
    """
...
def transfer_model_weights(trained_model, untrained_model):
    """
    Transfer weights from a trained model to a untrained model
    @param trained_model: trained model
    @param untrained_model: untrained model
    @return: the untrained-model with the trained weights
    """

    trained_weights = trained_model.trainable_weights
    untrained_weights = untrained_model.trainable_weights

    for weights in range(len(untrained_weights)):
        untrained_weights[weights].assign(trained_weights[weights])
