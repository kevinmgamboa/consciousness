...
def get_Spectrogram(audio, Fs, fft_size=200, step_size=1, spec_thresh=2.5, window_type=0):
    ...
    return abs(move_spec(
        aid_spectrogram(audio.astype('float64'), fft_size=fft_size, window_type=0, step_size=step_size, log=True,
                        thresh=spec_thresh))) / 2.5
...
def stft(X, fftsize=40, step=66, mean_normalize=True, real=False, compute_onesided=True, window_type=0):
    ...
    if window_type == 3:
        ...
        win = minR + ampR * (0.05 * np.sin(2 * np.pi * np.arange(size) * 3 * f)
                             + 0.4 * np.sin(2 * np.pi * np.arange(size) * f) + 0.25 * np.sin(
                    2 * np.pi * np.arange(size) * 2 * f + 45))
...
def raw_chunks_to_spectrograms(data, sf=100):
    ...
    for sample_idx in tqdm(range(len(data))):
        ch1 = aid_spectrogram(data[sample_idx][0].astype('float64'),
                              fft_size=fft_size, step_size=step_size, log=True, thresh=spec_thresh)
        ch2 = aid_spectrogram(data[sample_idx][1].astype('float64'),
                              fft_size=fft_size, step_size=step_size, log=True, thresh=spec_thresh)
